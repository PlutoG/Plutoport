...
target.special=special;var v=target.value;window.current_list_for=target;var url='//data/namelike.php?id='+encodeURIComponent(v);if(special)url=url+'&special='+special;if(typeof window.do_ajax_request!='undefined')do_ajax_request(url+keep_stub(),update_ajax_member_list_response);}
function update_ajax_member_list_response(result,list_contents)
{if(!list_contents)return;if(window.current_list_for==null)return;close_down();var data_list=(typeof document.createElement('datalist').options!='undefined');var list=document.createElement(data_list?'datalist':'select');list.className='people_list';list.setAttribute('id','ajax_list');if(data_list)
...
{if(typeof event=='undefined')var event=window.event;if(key_pressed(event,[40,38],true))
{if(typeof event.preventDefault!='undefined')event.preventDefault();return cancel_bubbling(event);}
return null;}
list.onkeypress=function(event)
{if(typeof event=='undefined')var event=window.event;if(key_pressed(event,[40,38,13],true))
{if(typeof event.preventDefault!='undefined')event.preventDefault();return cancel_bubbling(event);}
return null;}
