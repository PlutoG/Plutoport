...
{if(stub.match(/\_(from|start)$/))
{var year=document.getElementById(stub+'_year');var month=document.getElementById(stub+'_month');var day=document.getElementById(stub+'_day');var to_stub=stub.replace(/\_from$/,'_to').replace(/\_start$/,'_end');var to_day=document.getElementById(to_stub+'_day');var to_month=document.getElementById(to_stub+'_month');var to_year=document.getElementById(to_stub+'_year');if((to_day)&&(to_month)&&(to_year)&&(!((to_year.selectedIndex>year.selectedIndex)||((to_year.selectedIndex==year.selectedIndex)&&((to_month.selectedIndex>month.selectedIndex)||((to_month.selectedIndex==month.selectedIndex)&&(to_day.selectedIndex>=day.selectedIndex)))))))
{to_day.selectedIndex=day.selectedIndex;to_month.selectedIndex=month.selectedIndex;to_year.selectedIndex=year.selectedIndex;}}}
if(typeof window.YAHOO!='undefined')
{function initialise_date_field(stub,calob,linkob,unlimited,mindate,maxdate)
{var container=document.getElementById('cal'+stub+'Container');if(window.a_date_field_is_on)
{container.style.display='none';if(window[calob]!=null)
{window[calob]=null;set_inner_html(container,'');}
window.a_date_field_is_on=false;}else
{var date=new Date();var thisMonth=document.getElementById(stub+'_month').options[document.getElementById(stub+'_month').selectedIndex].value;if(thisMonth=='')
thisMonth=(date.getMonth()+1)+'';var thisYear=document.getElementById(stub+'_year').options[document.getElementById(stub+'_year').selectedIndex].value;if(thisYear=='')
thisYear=(date.getFullYear())+'';var thisDay=document.getElementById(stub+'_day').options[document.getElementById(stub+'_day').selectedIndex].value;if(thisDay=='')
thisDay=(date.getDate())+'';var monthDate=thisMonth+'/'+thisYear;var date=thisMonth+'/'+thisDay+'/'+thisYear;window[calob]=new YAHOO.widget.Calendar('cal'+stub,'cal'+stub+'Container',monthDate,date);if(!unlimited)
{if(mindate)
{window[calob].minDate=mindate;}else
{window[calob].minDate=new Date();window[calob].minDate.setDate(1);window[calob].minDate.setMonth(0);var year_field=document.getElementById(stub+'_year');var min_year_a=window.parseInt(year_field.options[1].value);var min_year_b=window.parseInt(year_field.options[year_field.options.length-1].value);var min_year=(min_year_a<min_year_b)?min_year_a:min_year_b;window[calob].minDate.setFullYear(min_year);}
if(maxdate)
{window[calob].minDate=maxdate;}}
window[calob].onSelect=function()
{var dates=window[calob].getSelectedDates();var date=dates[0];var year=document.getElementById(stub+'_year');var new_year_index=null;for(var i=1;i<year.options.length;i++)
{if(window.parseInt(year.options[i].value)==date.getFullYear())
{new_year_index=i;break;}}
if(new_year_index==null)
{var new_option=document.createElement('option');new_option.value=date.getFullYear();set_inner_html(new_option,date.getFullYear());year.appendChild(new_option);year.selectedIndex=year.options.length-1;}else
{year.selectedIndex=new_year_index;}
var month=document.getElementById(stub+'_month');month.selectedIndex=date.getMonth()+1;var day=document.getElementById(stub+'_day');day.selectedIndex=date.getDate();day.onchange(null);window[linkob].onclick();match_calendar_from_to(stub);};container.style.top=(find_pos_y(window[linkob])+find_height(window[linkob]))+"px";container.style.left=find_pos_x(window[linkob])+"px";container.style.position='absolute';container.style.width='150px';container.style.display='block';container.style.zIndex=150;window[calob].render();window.a_date_field_is_on=true;}}
...
return d;};this.subtract=function(date,field,amount){return this.add(date,field,(amount*-1));};this.before=function(date,compareTo){var ms=compareTo.getTime();if(date.getTime()<ms){return true;}else{return false;}};this.after=function(date,compareTo){var ms=compareTo.getTime();if(date.getTime()>ms){return true;}else{return false;}};this.getJan1=function(calendarYear){return new Date(calendarYear,0,1);};this.getDayOffset=function(date,calendarYear){var beginYear=this.getJan1(calendarYear);var dayOffset=Math.ceil((date.getTime()-beginYear.getTime())/this.ONE_DAY_MS);return dayOffset;};this.getWeekNumber=function(date,calendarYear){if(!calendarYear){calendarYear=date.getFullYear();}
...
{var cell=document.createElement("th");YAHOO.widget.Calendar_Core.setCssClasses(cell,[this.Style.CSS_WEEKDAY_CELL]);set_inner_html(cell,this.Options.LOCALE_WEEKDAYS[i]);row.appendChild(cell);}
...
this.resetRenderers();this.cellDates.length=0;var workingDate=YAHOO.widget.DateMath.findMonthStart(this.pageDate);this.renderHeader();this.renderBody(workingDate);this.renderFooter();this.onRender();};YAHOO.widget.Calendar_Core.prototype.renderHeader=function(){set_inner_html(this.headerCell,"");var headerContainer=document.createElement("div");headerContainer.className=this.Style.CSS_HEADER;headerContainer.appendChild(document.createTextNode(this.buildMonthLabel()));this.headerCell.appendChild(headerContainer);};YAHOO.widget.Calendar_Core.prototype.renderBody=function(workingDate){this.preMonthDays=workingDate.getDay();if(this.Options.START_WEEKDAY>0){this.preMonthDays-=this.Options.START_WEEKDAY;}
...
cal.cells=null;cal.tbody=null;cal.oDomContainer=null;cal.table=null;cal.headerCell=null;cal=null;};YAHOO.widget.Calendar_Core.prototype.renderOutOfBoundsDate=function(workingDate,cell){YAHOO.widget.Calendar_Core.addCssClass(cell,"previous");set_inner_html(cell,workingDate.getDate());return YAHOO.widget.Calendar_Core.STOP_RENDER;}
...
this.titleDiv.className="title";set_inner_html(this.titleDiv,this.title);if(this.outerContainer.style.position=="absolute")
{var linkClose=document.createElement("a");linkClose.href="javascript:void(null)";YAHOO.util.Event.addListener(linkClose,"click",this.hide,this);var imgClose=document.createElement("img");imgClose.src='http://localhost:47370//themes/default/images/date_chooser/calx.gif'.replace(/^https?:/,window.location.protocol);imgClose.className="close-icon";linkClose.appendChild(imgClose);this.linkClose=linkClose;this.titleDiv.appendChild(linkClose);}
