...
{window.faux_showModalDialog(maintain_theme_in_link('http://localhost:47370//data/page_link_chooser.php'+keep_stub(true)),null,'dialogWidth=600;dialogHeight=400;status=no;unadorned=yes',function(result)
...
{result=do_ajax_request('http://localhost:47370//data/autosave.php?type=retrieve'+keep_stub(),false,'key='+window.encodeURIComponent(cookie_name));if((result)&&(result.responseText)&&(posting_form.elements[i].value.length<result.responseText.length))
...
var cookie_name='ocp_autosave_'+window.location.pathname+window.location.search.replace(/[\?&]redirect=.*/,'')+':'+target.name;cookie_name=cookie_name.replace(/[\.=,; \t\r\n\013\014\/?]/g,'');var value=get_textbox(target)+String.fromCharCode(event.keyCode?event.keyCode:event.charCode);set_cookie(encodeURIComponent(cookie_name),'1',1);require_javascript('javascript_ajax');do_ajax_request('//data/autosave.php?type=store'+keep_stub(),function(){},'key='+window.encodeURIComponent(cookie_name)+'&value='+window.encodeURIComponent(value));window.last_autosave=this_date;}